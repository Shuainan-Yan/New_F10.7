...
class Exp_Main(Exp_Basic):
    ...
    def test(self, setting, test=0):
        ...
        ### ysn；将未反归一化smooth的数据写入文件
        true_file = "./CSV/"+self.args.model+"_Scale_smooth_true_lookback" + str(self.args.seq_len) + "_horizon" + str(
            self.args.pred_len) + ".csv"
        pred_file = "./CSV/"+self.args.model+"_Scale_smooth_pred_lookback" + str(self.args.seq_len) + "_horizon" + str(
            self.args.pred_len) + ".csv"
        ...
        ### ysn；将反归一化的数据写入文件
        new_true_file = "./CSV/"+self.args.model+"_Inverse_smooth_true_lookback" + str(self.args.seq_len) + "_horizon" + str(
            self.args.pred_len) + ".csv"
        new_pred_file = "./CSV/"+self.args.model+"_Inverse_smooth_pred_lookback" + str(self.args.seq_len) + "_horizon" + str(
            self.args.pred_len) + ".csv"
        write_csv(new_true_file, smooth_trues)
        write_csv(new_pred_file, new_preds)

        print("-----------------------------------------反归一化后的指标（含单位）---------------------------------------------")
        mae, mse, rmse, mape, mspe = metric(new_preds,  smooth_trues)
        print('rmse:{}, mse:{}, mae:{}, mape:{}'.format(rmse, mse, mae, mape))
